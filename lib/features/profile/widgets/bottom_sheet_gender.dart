...
class BottomSheetGender extends StatefulWidget {
  ...
  final List<InfoItem> gender;
  final Function(InfoItem) onTap;

  const BottomSheetGender({super.key, required this.selectedValue, required this.onTap, required this.gender});
            ...
            child: LeftAlignedColumn(
                children: [
                  Stack(
                    children: [
                      Center(
                        child: TextWidget.jakartaSansSemiBold('Select Your Gender', size: 16, color: ColorWidget.blackColor),
                      ),
                      Align(
                          ...
                          children: [
                            Expanded(
                              child: GestureDetector(
                                onTap: () {
                                  setState(() {
                                    widget.onTap(widget.gender[index]);
                                  });
                                  context.maybePop();
                                },
                                child: TextWidget.jakartaSansRegular(widget.gender[index].content ?? '', size: 16, color: ColorWidget.iconColor).horizontalPadded().verticalPadded(8),
                              ),
                            ),
                            if (widget.selectedValue.title == widget.gender[index].title) Icon(Icons.check, size: 32, color: ColorWidget.accentColor)
                          ],
                        );
                      },
